...
//  public $_api = false;
    ...
    function __construct() {
        ...
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'class-JG_Cache.php');
...
//	add_action( 'admin_notices', array($this->MCAPI, 'showMessages') );
        ...
        add_action('wp_ajax_sanitize', [$this, 'sanitize_callback']);
    ...
    function chimpxpressAddAdminHead() {
        ...
        // add js files
        wp_enqueue_script('chimpxpress', plugins_url('js' . DS . 'jquery.colorbox-min.js', __FILE__));
    }
    ...
    function compose_clear_cache_callback() {
        ...
        if ($wp_filesystem->method == 'direct') {
            $wp_filesystem->delete(ABSPATH . 'wp-content' . DS . 'plugins' . DS . 'chimpxpress' . DS . 'cache', true);
        } else {
            $ftpstream = ftp_connect($this->settings['ftpHost']);
            ftp_login($ftpstream, $this->settings['ftpUser'], $this->settings['ftpPasswd']);
            ftp_chdir($ftpstream, $this->settings['ftpPath']);

            $dir = 'wp-content' . DS . 'plugins' . DS . 'chimpxpress' . DS . 'cache';
    ...
    public function compose_gotoStep_callback() {
        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $this->optionsName . '-compose')) {
            die('Invalid request!');
        }

        include(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'compose.php');
        exit;
    }
    ...
    public function compose_removeDraft_callback() {
        ...
        $cid = preg_replace('/[^a-z0-9]/', '', $_POST['cid']);

        $this->MCAPI->campaigns(['campaign_id' => $cid], 'DELETE');

        if (is_dir(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'tmp')) {
            global $wp_filesystem;
            if ($wp_filesystem->method == 'direct') {
                $wp_filesystem->delete(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'tmp', true);
            } else {
                $ftpstream = ftp_connect($this->settings['ftpHost']);
                ftp_login($ftpstream, $this->settings['ftpUser'], $this->settings['ftpPasswd']);
                ftp_chdir($ftpstream, $this->settings['ftpPath']);
                $this->ftp_delAll($ftpstream, 'wp-content' . DS . 'plugins' . DS . 'chimpxpress' . DS . 'tmp');
                ftp_close($ftpstream);
            }
        }
        exit;
    ...
    function import_callback() {
        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $this->optionsName . '-import')) {
        ...
        if ($type == 'post') {
            ...
            if ($_POST['datatype'] == 'html') {
                ...
                require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'class-css_to_inline_styles.php');
            ...
            */
            ...
            if ($this->settings['GAprofile']) {
                $script = "\n<script type=\"text/javascript\">\n" .
                    "var _gaq = _gaq || [];\n" .
                    "_gaq.push(['_setAccount', '" . $this->settings['GAprofile'] . "']);\n" .
    ...
    function load_campaigns_callback() {
        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $this->optionsName . '-load-campaigns')) {
            die('Invalid request!');
        }
    ...
    function archive_deleteLP_callback() {
        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $this->optionsName . '-archive')) {
            die('Invalid request!');
        }
    ...
    public function ftp_find_root() {
        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $this->optionsGroup . '-options')) {
            die('Invalid request!');
        }
    ...
    public function main() {
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'main.php');
    }
    ...
    function compose() {
        ...
        if (!is_dir(ABSPATH . $cacheDir)) {
            if (!$useFTP) {
                ...
                $handler = ftp_connect($this->settings['ftpHost']);
                $login = ftp_login($handler, $this->settings['ftpUser'], $this->settings['ftpPasswd']);
                ftp_chdir($handler, $this->settings['ftpPath']);
        ...
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'compose.php');
        echo '</div>';
    }

    function import() {
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'import.php');
    }

    function archive() {
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'archive.php');
    }

    function editLP() {
        require_once(WP_PLUGIN_DIR . DS . 'chimpxpress' . DS . 'editLP.php');
    }

    public function options() { ?>
        <style type="text/css">
            /*
            #wp_chimpxpress table tr th a {
            cursor:help;
            }
            .large-text{width:99%;}
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="text" name="<?php echo esc_attr($this->optionsName); ?>[apiKey]"
                                   ...
                                   value="<?php echo esc_attr($this->settings['apiKey'] ?? ''); ?>"
                                   id="<?php echo esc_attr($this->optionsName); ?>_apikey" class="regular-text code"/>

                            <script type="text/javascript">
                                jQuery(document).ready(function ($) {
                                    if (jQuery('#<?php echo esc_attr($this->optionsName); ?>_apikey').val() == '') {
                            ...
                            <div>
                                <ol id="mc_apikey" style="display:none; list-style-type:decimal; margin-top: 1em;">
                                    <li>
                                        <?php echo esc_html(sprintf(__('You need a Mailchimp account. If you do not have one, <a href="%s" target="_blank">sign up for free</a>', 'chimpxpress'), 'https://www.mailchimp.com/signup')); ?>
                                    </li>
                                    <li>
                                        <?php echo esc_html(sprintf(__('<a href="%s" target="_blank">Grab your API Key</a>', 'chimpxpress'), 'http://admin.mailchimp.com/account/api-key-popup')); ?>
                                    </li>
                                </ol>
                            </div>
                        </td>
		    ...
		    */ ?>
                    ...
                    <?php if ($_SESSION['MCping']) { ?>
                        <tr valign="top">
                            ...
                            <td>
                                <span style="font-size:12px;"><?php echo esc_html($_SESSION['MCusername']); ?></span>
                            </td>
                        </tr>
                    <?php } ?>
                    <tr valign="top">
				...
				<?php echo ($_SESSION['MCping'])
                    ? '<span style="color:green;">' . esc_html($_SESSION['MCping']) . '</span>'
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="text" name="<?php echo esc_attr($this->optionsName); ?>[ftpHost]"
                                   style="text-align:center;width:320px;" maxlength="36"
                                   value="<?php echo esc_attr($this->settings['ftpHost']); ?>"
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="text" name="<?php echo esc_attr($this->optionsName); ?>[ftpUser]"
                                   style="text-align:center;width:320px;" maxlength="36"
                                   value="<?php echo esc_attr($this->settings['ftpUser']); ?>"
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="text" name="<?php echo esc_attr($this->optionsName); ?>[ftpPath]"
                                   ...
                                   value="<?php echo esc_attr($this->settings['ftpPath']); ?>"
                                   id="<?php echo esc_attr($this->optionsName); ?>_ftpPath" class="regular-text code"/>
                            &nbsp;&nbsp;
                            <a href="javascript:ftp_find_root()"><?php esc_html_e('Find FTP Path', 'chimpxpress'); ?></a>
                            <script type="text/javascript">
                                function ftp_find_root() {
                                    var data = {
                                        action: 'ftp_find_root',
                                        _wpnonce: jQuery('#_wpnonce').val(),
                                        ftpHost: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpHost').val(),
                                        ftpUser: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpUser').val(),
                                        ftpPasswd: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpPasswd').val()
                                    };
                                    jQuery.post(ajaxurl, data, function (response) {
                                        jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpPath').val(response);
                                    });
                                }
                            </script>
                            <a class="chimpxpress_help" title="<?php esc_html_e('Click for Help!', 'chimpxpress'); ?>"
                            ...
                            <script type="text/javascript">
                                function testFtp() {
                                    ...
                                    var data = {
                                        ...
                                        ftpUser: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpUser').val(),
                                        ftpPasswd: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpPasswd').val(),
                                        ftpPath: jQuery('#<?php echo esc_attr($this->optionsName); ?>_ftpPath').val()
                                    };
                                    jQuery.post(ajaxurl, data, function (response) {
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="text" name="<?php echo esc_attr($this->optionsName); ?>[GAprofile]"
                                   style="text-align:center;width:320px;" maxlength="36"
                                   value="<?php echo esc_attr($this->settings['GAprofile']); ?>"
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <input type="submit" name="Submit" class="button"
                                   value="<?php esc_html_e('Update Settings &raquo;', 'chimpxpress'); ?>"/>
                        </td>
                    </tr>
                </table>
            </form>
    ...
    function chimpxpressLoadLanguage() {
        if (function_exists('load_plugin_textdomain')) {
            load_plugin_textdomain($this->optionsName, false, 'chimpxpress' . DS . 'languages');
        }
    }
