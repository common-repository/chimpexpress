...
class chimpxpressMCAPI {
    ...
    private function log($msg) {
        if ($this->debug && trim($msg)) {
            //error_log($msg);
            // using file_put_contents directly instead of WP_Filesystem since FILE_APPEND mode is not implemented
...
if (!function_exists('debug_wp_remote_post_and_get_request')) {
    function CX_debug_wp_remote_request($response, $context, $class, $request, $url) {
    ...
    add_action('http_api_debug', 'CX_debug_wp_remote_request', 10, 5);
}
